...
func NewSocket(socketPath string) (*Socket, error) {
...
func (s *Socket) Recv(bufferSize int, timeout time.Duration) (string, error) {
	...
	for {
		...
		if strings.Contains(respStr, "MICA-SUCCESS") {
			...
			if msg != "" {
				fmt.Println(msg)
				// NOTICE: in the simple test file `gdb`
			}
...
func SendCtrlMsg(command, client string) error {
	...
	socket, err := NewSocket(ctrlSocket)
	if err != nil {
		return err
	}
	defer socket.Close()
...
func QueryStatus() error {
	...
	for _, file := range files {
		...
		socket, err := NewSocket(filepath.Join(SocketPath, file.Name()))
		if err != nil {
			continue
		}
